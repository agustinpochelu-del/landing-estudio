/**
 * Lector mínimo de planillas .xlsx, .xlsm y .csv, sin dependencias.
 *
 * Un .xlsx es un ZIP con XML adentro, y un .xlsm es el mismo ZIP con el
 * proyecto de macros adentro: se lee igual, la macro se ignora. Acá se abre
 * el ZIP a mano y se
...
async function leerPlanilla(archivo) {
  ...
  if (nombre.endsWith('.xls')) {
    throw new Error('El formato .xls viejo no se puede leer. Guardalo como .xlsx, .xlsm o .csv desde Excel.');
  }
  return leerXlsx(await archivo.arrayBuffer());
}

/* Copiado tal cual del importador de Libro de IVA (`../LIVA D/xlsx.js`).
   Si se arregla un caso ahí, se copia acá; no se bifurca. */
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { leerPlanilla, leerXlsx, leerCsv, columnaDeReferencia, serialAFecha };
}
if (typeof window !== 'undefined') {
  window.Planilla = { leerPlanilla, leerXlsx, leerCsv };
}
